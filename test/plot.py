...
def parse_args():
    ...
    parser.add_argument(
        '--save',  type=str, default='', help='filename of output')
...
def read_data(args):
    ...
    data[args.metric] = data[args.metric].apply(TRANSFORMS[args.transform]) / scale
    return data
...
def plot_groups(label_groups, args):
    ...
    for label, group in label_groups.items():
        if label.find("MyMap") >= 0 : label = "OrderStatisticMap"
        elif label.find("Pooled") >= 0  : label = "pooled std::map"
        elif label.find("StdMap") >= 0  : label = "std::map"
        elif label.find("StdUnorderedMap") >= 0  : label = "std::unordered_map"

        plt.plot(group['input'], group[args.metric], '--o', label=label)
    if args.logx:
        plt.xscale('log')
    if args.logy:
        plt.yscale('log')
    ...
    if args.save != "" : plt.savefig(args.save, format="svg")
    else : plt.show()
